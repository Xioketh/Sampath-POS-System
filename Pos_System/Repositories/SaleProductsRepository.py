...
class SaleProductsRepository:
    def add_Sale_products(self, saleId, totalProductQty, totalProductAmount, product_id, unitPrice):
    ...
    def getAllTimeTrendProduct(self):
        conn = sqlite3.connect('SampathPOS.db')
        cursor = conn.cursor()
        cursor.execute('''
    ...
    def getTrendProductFromDateRange(self, from_date, to_date):
        conn = sqlite3.connect('SampathPOS.db')
        cursor = conn.cursor()
        cursor.execute('''
                    SELECT pl.name,s.branchName, COUNT(productId) as product_count
    ...
    def getSaleProductByProID(self, productID):
        conn = sqlite3.connect('SampathPOS.db')
        cursor = conn.cursor()
        cursor.execute('''
